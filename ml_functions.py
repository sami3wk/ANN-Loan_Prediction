class Missing_data:
    ...
    def preprocessing(self,X):
        ...
        X2 = pd.get_dummies(X[cat],dtype=int)
        X3 = X1.join(X2)
        return X3

    
    def convert_to_labelEncoder(self,df):
        from pandas import DataFrame
        from sklearn.preprocessing import LabelEncoder
        le = LabelEncoder()
        df = DataFrame(le.fit_transform(df),columns=[df.columns[0]])
        return df,le
...
class EDA:
    ...
    def bivariate_ycat(self,df,y_df,cat,con):
        ...
        for i in df.columns:
            if(df[i].dtypes == "object"):
                ...
                plt.subplot(u,3,x)
                sb.boxplot(x=y_df[y_fin],y=df[i])
                x = x + 1
    ...
    def bivariate_ycon(self,df,y_df,cat,con):
        ...
        for i in df.columns:
            if(df[i].dtypes == "object"):
                plt.subplot(u,3,x)
                sb.boxplot(x=df[i],y=y_df[y_fin])
                x = x + 1
            else:
                plt.subplot(u,3,x)
                sb.scatterplot(x=df[i],y=y_df[y_fin])
                x = x + 1
...
class NeighborsKNN:
    ...
    def KNN_no_Neighbors_reg(self,max_neighbors,xtrain,ytrain):
        from sklearn.model_selection import GridSearchCV
        from sklearn.neighbors import KNeighborsRegressor
        tg = {"n_neighbors":range(2,max_neighbors,1)}
        knn = KNeighborsRegressor()
        cv = GridSearchCV(knn,tg,scoring="neg_mean_squared_error",cv=4)
        cvmodel = cv.fit(xtrain,ytrain)
        return cvmodel.best_params_
    
    def KNN_no_Neighbors_classi(self,max_neighbors,xtrain,ytrain):
        from sklearn.model_selection import GridSearchCV
        from sklearn.neighbors import KNeighborsClassifier
        tg = {"n_neighbors":range(2,max_neighbors,1)}
...
class Best_Alpha:
    ...
    def ridge_alpha(self,xtrain,ytrain):
        from sklearn.linear_model import RidgeCV
        rr = RidgeCV()
        model = rr.fit(xtrain,ytrain)
        return model.alpha_
    
    def lasso_alpha(self,xtrain,ytrain):
